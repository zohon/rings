...
const RingComponent = class {
    ...
    renderRings({ rings, pos }) {
        return index.h("div", { class: `rings ${this.checkDraggable(rings) ? 'dropable' : ''}`, "data-max": Math.max(0, ...rings) > 0 ? Math.max(0, ...rings) : '', onDrop: event => this.dropHandler(event, pos), onDragOver: event => this.dragOver(event, rings) }, rings.map((number) => this.renderRing(number)));
    }
    renderRing(number, draggable = false) {
        return index.h("div", { class: `ring ${draggable ? 'draggable' : ''}`, key: number, "data-player": number % 2 == 0 ? 1 : 2, "data-number": number, draggable: draggable, onDragStart: (event) => this.dragStart(event, number), onDragEnd: () => this.dragEnd() });
    }
    renderBoard() {
        return index.h("div", { class: "board" }, this.gameBoard.sort(this.sortPos).map(item => {
    ...
    renderPLayer(playerNumber) {
        ...
        return index.h("div", { class: "player", "data-player": playerNumber }, availableRIngs.map(number => this.renderRing(number, draggable)));
    }
    render() {
        return (index.h(index.Host, { key: '049ead70783f1fb267d9762f05639ab03ab87dd0' }, index.h("div", { key: '10960cb802afe83acd465e579537dc302c2af712', class: "header" }), index.h("div", { key: '2a262e1c51548556bfa40d632f749b5f495ffd1b', class: "game" }, this.renderPLayer(1), this.renderBoard(), this.renderPLayer(2))));
    }
    static get watchers() { return {
        "gameBoard": ["checkBoard"]
    }; }
};
RingComponent.style = RingComponentStyle0;

exports.ring_component = RingComponent;

//# sourceMappingURL=ring-component.cjs.entry.js.map